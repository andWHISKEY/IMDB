...
<br><br>
<input type="delete_movie_id" class="form-control" name="movie_name" id="delete_movie_id" placeholder="Enter movie ID here (e.g., 0119177)" required>
</div>
<br><br>
         ...
         <!-- MOVIES table -->
       	<table class="table table-striped" cellspacing="0" width="100%">
       		...
       		<tbody>
					...
					<!--무비 선택한 경우-->
					<?php
							#대소문자 구분 안하는 select 속성
							#포함되도 검색되도록 like
							if(!strcmp($_POST['search'],"actor")){//actor 선택
								 $sql="select MOVIES.Id,MOVIES.Name,MOVIES.Year,MOVIES.Runtime,MOVIES.rating,MOVIES.Plot,MOVIES.Budget,MOVIES.Gross_glob,MOVIES.Submitter_id from MOVIES,ROLES,ACTORS where ACTORS.Name LIKE '%".$_POST['Search_name']."%'and ACTORS.Id=ROLES.Actor_id and ROLES.Movie_id=MOVIES.Id";
							 ...
							 if(!strcmp($key,"Name")){
								 print '<td>
								 <form method="post">
									 <a href="movie_description.php">'.$val.'</a>
							...
							else if(!strcmp($_POST['search'],"genre")){
								 $sql="select MOVIES.Id,MOVIES.Name,MOVIES.Year,MOVIES.Runtime,MOVIES.rating,MOVIES.Plot,MOVIES.Budget,MOVIES.Gross_glob,MOVIES.Submitter_id from MOVIES,MOVIE_GENRES where MOVIE_GENRES.Genre LIKE '%".$_POST['Search_name']."%' and MOVIE_GENRES.Movie_id=MOVIES.Id";
								 $result = mysqli_query($link, $sql);
								 while ($row = mysqli_fetch_assoc($result)) {
									print "<tr>";
									foreach ($row as $key => $val){
							...
							else if(!strcmp($_POST['search'],"director")){
								 $sql="select MOVIES.Id,MOVIES.Name,MOVIES.Year,MOVIES.Runtime,MOVIES.rating,MOVIES.Plot,MOVIES.Budget,MOVIES.Gross_glob,MOVIES.Submitter_id from MOVIES,DIRECTORS,MOVIE_DIRECTORS where DIRECTORS.Name like '%".$_POST['Search_name']."%'and DIRECTORS.Id=MOVIE_DIRECTORS.Director_id and MOVIE_DIRECTORS.Movie_id=MOVIES.Id";
								 $result = mysqli_query($link, $sql);
								 while ($row = mysqli_fetch_assoc($result)) {
									print "<tr>";
									foreach ($row as $key => $val){
							...
							else if(!strcmp($_POST['search'],"movie")){
						$sql = "select * from MOVIES WHERE NAME LIKE '%".$_POST['Search_name']."%'";
							 $result = mysqli_query($link, $sql);
							 while ($row = mysqli_fetch_assoc($result)) {
									print "<tr>";
									foreach ($row as $key => $val){
