...
document.getElementById("delete").onclick = function() {
    ...
    document.getElementById("all_description").style.display="none";
...
document.getElementById("btn-delete").onclick = function() {
    ...
    document.getElementById("all_description").style.display="block";
...
<!-- MOVIES table -->
           <?php
                  ...
                  //print var_dump($row['Id']);
                  //echo $row['Name']
                ?>

                <h2 class="movie_desc"><?php echo $row['Name'] ?></h2>
                <h3 class="movie_desc">Rate: <?php echo $row['rating'] ?></h3>

                <p class="movie_desc">Year: <?php echo $row['Year'] ?></p>
                <p class="movie_desc">Runtime: <?php echo $row['Runtime'] ?>분</p>
                <h4 class="movie_desc">Plot</h4>
                <p class="movie_desc"> <?php echo $row['Plot'] ?></p>
                <?php
                $sql='select Name from ROLES,ACTORS where Movie_id="'.$movie_id.'" AND ROLES.Actor_id=ACTORS.Id';
                $result = mysqli_query($link, $sql);

                echo "<h4 class='movie_desc'>Actors</h4>";
                echo "<ul class='movie_desc'>";
                ...
                <?php
                $sql='select Name from DIRECTORS,MOVIE_DIRECTORS where Movie_id="'.$movie_id.'" AND MOVIE_DIRECTORS.director_id=DIRECTORS.Id';
                $result = mysqli_query($link, $sql);

                echo "<h4 class='movie_desc'>Director</h4>";
                echo "<ul class='movie_desc'>";
                $row = mysqli_fetch_assoc($result);
                print "<li>".$row['Name']."</li>";
                ...
                <?php
                $sql='select Genre from MOVIE_GENRES where Movie_id="'.$movie_id.'"';
                $result = mysqli_query($link, $sql);

                echo "<h4 class='movie_desc'>Genre</h4>";
                ...
                <?php
                $sql='select Reviewer_id,Rating,Review_text from REVIEWS where Movie_id="'.$movie_id.'"';
                $result = mysqli_query($link, $sql);

                echo "<h4 class='movie_desc'>Reviews</h4>";
                while($row = mysqli_fetch_assoc($result)){
                   print "<p class='movie_desc'>".$row['Reviewer_id']."&nbsp;(&nbsp;&nbsp;".$row['Rating']."&nbsp;&nbsp;)</p></br>";
                   print "<p class='movie_desc'><textarea readonly rows='10' cols='100'>".$row['Review_text']."</textarea></p>";
                }
                ?>
             </br><hr></br>

              <form action="review_update.php" method="post">
                  <p class="movie_desc"><input type="text" name="Reviewer_id" placeholder="Your_id" />
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                     <input type="text" name="Rating" placeholder="Rating" /></p>
                    <p class="movie_desc"><textarea input type="text" name="Review_text" placeholder="Enter your review here" rows='10' cols='100'></textarea></p>
                    <input type="hidden" name="Movie_id" value="<?php echo $movie_id ?>">
                    <button type="submit" class="btn-default">Add review</button>
              </form>
</div>
